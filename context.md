...
* **Descobertas e Integração Aplicada**:
  ...
  - **Autenticação e Headers do Exporter:** Mapeamos que o coletor do Arize Phoenix exige a autenticação via cabeçalho `"Authorization": f"Bearer {phoenix_api_key}"` nos headers de requisição OTLP, validando a telemetria com sucesso.
  - **Transmissão Imediata (Force Flush em Background):** Devido à execução concorrente da Crew em threads de background do FastAPI via `background_tasks.add_task`, implementamos o método singleton de rastreio `flush_traces()` invocando `provider.force_flush()`. Esse método é chamado de forma resiliente no bloco `finally` da thread em [`app/jobs/manager.py`](file:///c:/Users/lemos/OneDrive/Área de Trabalho/Customer-support-crew/app/jobs/manager.py), garantindo integridade e transmissão em tempo real antes do encerramento da thread.
