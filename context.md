...
* **Descobertas e Integração Aplicada**:
  ...
  - **Resolução de Conflitos OTel (Ordem de Inicialização):** Para evitar o erro `Overriding of current TracerProvider is not allowed` (causado pelo CrewAI registrando um TracerProvider antes do Phoenix), ativamos `set_global_tracer_provider=False` na chamada `register()` e movemos a chamada de `setup_instrumentation()` para o topo absoluto do [`app/main.py`](file:///c:/Users/lemos/OneDrive/Área de Trabalho/Customer-support-crew/app/main.py), antes de qualquer outro import de rotas ou orquestração.
